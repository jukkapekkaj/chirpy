...
const updateUseInformation = `-- name: UpdateUseInformation :exec
UPDATE users SET email=$1, hashed_password=$2, updated_at=NOW() WHERE id=$3
`

type UpdateUseInformationParams struct {
	Email          string
	HashedPassword string
	ID             uuid.UUID
}

func (q *Queries) UpdateUseInformation(ctx context.Context, arg UpdateUseInformationParams) error {
	_, err := q.db.ExecContext(ctx, updateUseInformation, arg.Email, arg.HashedPassword, arg.ID)
	return err
}
